#!/usr/bin/env python3
"""
editor_script.jsを生成するスクリプト（ページ送り機能付き）
"""

js_content = '''// Gold Standard Editor JavaScript with Pagination
    ...
    data.persons.forEach((person, index) => {
        ...
        card.innerHTML = `
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    <label>民族性 (Ethnicity)</label>
                    <input type="text" value="${person.ethnicity || ''}"
                           onchange="data.persons[${index}].ethnicity = this.value">
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    <label>神格タイプ</label>
                    <input type="text" value="${person.divinity_type || ''}"
                           onchange="data.persons[${index}].divinity_type = this.value">
                </div>
            </div>
            <div class="form-group">
                <label>神格分類の理由</label>
                <textarea rows="2" onchange="data.persons[${index}].divinity_classification_reasoning = this.value">${person.divinity_classification_reasoning || ''}</textarea>
            </div>
            ` : ''}
    ...
    return benefactions.map((benef, benefIndex) => `
        <div class="benefaction-card">
            ...
            <div class="form-group">
                ...
                <textarea rows="2" onchange="data.persons[${personIndex}].benefactions[${benefIndex}].cost_conversion_reasoning = this.value">${benef.cost_conversion_reasoning || ''}</textarea>
            </div>
            <div class="form-group">
                <label>備考</label>
                <textarea rows="2" onchange="data.persons[${personIndex}].benefactions[${benefIndex}].notes = this.value">${benef.notes || ''}</textarea>
            </div>
        </div>
    `).join('');
}
...
with open('editor_script.js', 'w', encoding='utf-8') as f:
    f.write(js_content)

print("editor_script.js を生成しました")
